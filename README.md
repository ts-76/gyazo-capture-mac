...
- 完全な黒塗り、ブラー、モザイク
- 元領域と拡大円を個別に移動でき、倍率も変更可能なルーペ
- クロップ、左右90度回転、Undo／Redo
- ファイル名を指定したPNG保存、クリップボードコピー、Gyazoアップロード
...
ビルド13以降は、画面収録の許可を更新後も引き継げるよう、Bundle IDに基づく固定のDesignated RequirementをAd Hoc署名へ埋め込んでいます。ビルド12以前から初めて更新するときだけ、古い許可をリセットして再許可する必要があります。

この固定要件はmacOSが同じアプリとして追跡するためのもので、Developer ID署名のように配布元の本人性を証明するものではありません。Gatekeeperによる初回起動の手順は引き続き必要です。
